...
$$
  y^*_{ij} \sim \text{Bernoulli}(p^*_{ij}).
$$  
...
$$
where $\kappa=0.5$ corresponds to no signal where it would be impossible to predict an outcome in advance (a coin flip). A $\kappa = 0}$ is a noiseless signal (**rpsfw**).
...
This loss function is essentially a [Brier Score](https://en.wikipedia.org/wiki/Brier_score) average over simulation replications.
...
rpsfw_df %>%
  mutate(
    outcome_k00 = add_noise_to_outcomes(outcome, error_prob = 0.00),
    outcome_k10 = add_noise_to_outcomes(outcome, error_prob = 0.10),
    outcome_k25 = add_noise_to_outcomes(outcome, error_prob = 0.50),
    outcome_k50 = add_noise_to_outcomes(outcome, error_prob = 0.50)
  ) %>%
  sample_n(10) %>%
  knitr::kable()
```
...
rpsfw_df_mod <-  do.call(
    "rbind",
    replicate(10, rpsfw_df, simplify = FALSE)
)
