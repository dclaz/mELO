...
for (k in 1:nrow(params_map)){
  ...
  print_time_status(k)
  print_time_status(params_map[k,])
  
  set.seed(231)
  n <- params_map$n[k] #500
  kappa <- params_map$kappa[k] #0.1
  ...
  #error_mat_df %>% filter(value == min(value))
...
pars <- error_mat_df %>% filter(value==min(value))
  model <- mELO(
    current_rpsfw_df_mod,
    k = 2,
    eta_1 = pars$eta_1,
...
good_c_mat <- model$c_mat

preds <- predict(
  model,
  current_test_df
)
data.frame(current_test_df, preds = preds %>% round(2)) %>%
  arrange(-abs(preds - outcome))
    
```

# Remedies

Some ideas that could be implemented to improve the mELO models:
